...
$pdf->AddPage();
  ...
  // $pdf->cell(180,7,"THIAM GENERAL BUSINESS",0,0,"C");
   ...
   $pdf->Ln(30);   
   $pdf->cell(180,7,utf8_decode("DETAIL DE LA VENTE N° ").utf8_decode($vente['num_com']),1,0,'C');
   $pdf->Ln(15);   
   ...
   foreach($details as $detail){
    ...
    $pdf->cell(30,7,utf8_decode(number_format($vente['prix_ht'],0,',','.' )),1,0,'C');
    $pdf->Ln();
    $pdf->cell(35,7,"",0,0,'C');
    $pdf->cell(75,7,"",0,0,'');
    $pdf->cell(15,7,"",0,0,'C');
    $pdf->cell(30,7,"TVA(18%)",1,0,'C');
    $pdf->cell(30,7,utf8_decode(number_format($vente['mnt_tva'],0,',','.' )),1,0,'C');
    $pdf->Ln();
    $pdf->cell(35,7,"",0,0,'C');
    $pdf->cell(75,7,"",0,0,'');
    $pdf->cell(15,7,"",0,0,'C');
    $pdf->cell(30,7,"Prix TTC",1,0,'C');
    $pdf->cell(30,7,utf8_decode(number_format($vente['prix_ttc'],0,',','.' )),1,0,'C');
    $pdf->Ln(20);


    $pdf->SetFont ( 'TIMES','B', 16 );
    $txtz = utf8_decode("Le montant total de la commande s'élève à: ");
    $pdf->cell(140,7,$txtz. number_format($vente['prix_ttc'],0,',','.' )." FCFA",0,0,'');
    $pdf->Ln(5);
    ...
    elseif($statu=="Impayé") {
        ...
        $pdf->cell(140,7,$txto,0,0,'');
        $pdf->Ln(5);
        $rest = $vente["prix_ttc"] - $vente["encaisser"];
